...
public abstract class MouseMixin {
	...
	private void spellengineextension$updateMouse(ClientPlayerEntity instance, double cursorDeltaX, double cursorDeltaY, Operation<Void> original) {
		if (SpellEngineExtensionConfigs.SERVER_CONFIG.enable_movement_locking_spell_casting.get()) {
			...
			if (player != null) {
				SpellCast.Process process = ((SpellCaster.Player) player).getSpellCastProcess();
				if (SpellEngineExtensionConfigs.SERVER_CONFIG.movement_locking_prevents_player_orientation_changes.get() && ((((DuckPlayerEntityMixin) player).spellengineextension$getMovementLockingTicks() > 0) || (process != null && process.spell().value().active.cast != null && process.spell().isIn(SpellEngineExtension.ENABLES_MOVEMENT_LOCKING_DURING_CASTING)))) {
					return;
				}
			}
		}
